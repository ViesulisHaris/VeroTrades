'use client';  // Make client for delete
...
export default function StrategyCard({ strategy, onDelete }: Props) {
  ...
  // Stats fetch (server-like, but client for delete)
  const [stats, setStats] = useState({ totalPnL: 0, winrate: '0', profitFactor: '0', trades: 0 });

  useEffect(() => {
    (async () => {
      const { data: trades } = await supabase.from('trades').select('pnl').eq('strategy_id', strategy.id);
  ...
  return (
    <div className="glass p-5 relative">
      ...
      <h3 className="text-lg font-semibold mb-2 text-metallic-silver">{strategy.name}</h3>

      <div className="grid grid-cols-2 gap-2 text-sm">
        <div><span className="text-metallic-dark">Winrate:</span> {stats.winrate}%</div>
        <div><span className="text-metallic-dark">Profit Factor:</span> {stats.profitFactor}</div>
        <div><span className="text-metallic-dark">Net PnL:</span> {formatCurrency(stats.totalPnL)}</div>
        <div><span className="text-metallic-dark">Trades:</span> {stats.trades}</div>
      </div>
